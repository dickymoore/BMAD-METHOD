...
description: "Interactive elicitation methods to refine content via selectable techniques"
web_bundle: false
---
...
<critical>MANDATORY: Execute ALL steps in the flow section IN EXACT ORDER</critical>
<critical>DO NOT skip steps or change the sequence</critical>
<critical>HALT immediately when halt-conditions are met</critical>
<critical>Each action xml tag within step xml tag is a REQUIRED action to complete that step</critical>
<critical>Sections outside flow (validation, output, critical-context) provide essential context - review and apply throughout execution</critical>
<critical>YOU MUST ALWAYS SPEAK OUTPUT in your Agent communication style with the `communication_language`</critical>
...
- Return the enhanced version when the user selects "x" to proceed
