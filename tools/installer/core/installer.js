...
class Installer {
  ...
  async install(originalConfig) {
    ...
    try {
      ...
      const previousSkillIds = this._getPreviousSkillIdsForCleanup(previousSkillManifestRows, config.preserveModules);
  ...
  async _removeDeselectedModules(existingInstall, config, paths) {
    ...
    const preservedModules = new Set(config.preserveModules || []);
    const toRemove = [...previouslyInstalled].filter((m) => !newlySelected.has(m) && !preservedModules.has(m) && m !== 'core');
  ...
  async _installAndConfigure(
    ...
    const configTask = {
      ...
      task: async (message) => {
        ...
        const preservedModules = config.preserveModules || originalConfig._preserveModules || [];
